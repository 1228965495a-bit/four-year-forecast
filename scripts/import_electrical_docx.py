...
MAJOR_ID = "electrical_engineering"
DOCX_PATH = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/Users/zhenzhen/Downloads/S08-电气工程-完整四年脚本补强包.docx")
...
MAJOR_STAT_MAP = {
    "电路脑": "circuitControl",
    "实验存活率": "smokeRisk",
    "国网执念": "stateGridObsession",
    "强电敬畏": "powerRespect",
}

ROUTE_MARKER_MAP = {
    "grid_runner": "electrical_engineering_route_state_grid",
    "new_energy_branch": "electrical_engineering_route_new_energy",
    "stable_survivor": "electrical_engineering_route_cross",
}

SPECIAL_ENDING_IDS = {
    "ending_electrical_engineering_mid_gg",
    "ending_electrical_engineering_transfer_success",
    "ending_electrical_engineering_transfer_fail",
    "ending_electrical_engineering_hidden",
}

PRESERVED_TIMELINE_IDS_BY_SEMESTER = {
    "y1s2": ["electrical_engineering_y1s2_transfer_005"],
    "y2s2": ["electrical_engineering_y2s2_transfer_011"],
    "y3s1": ["electrical_engineering_y3s1_route_015"],
    "y3s2": ["electrical_engineering_y3s2_route_016", "electrical_engineering_y3s2_route_017"],
    "y4s1": ["electrical_engineering_y4s1_route_020", "electrical_engineering_y4s1_gg_check_021"],
    "y4s2": ["electrical_engineering_y4s2_settlement_023"],
}

MAJOR_STAT_SCALE = {
    "circuitControl": 3,
    "smokeRisk": 3,
    "stateGridObsession": 3,
    "powerRespect": 3,
}
...
def iter_stat_pairs(text: str) -> list[tuple[str, int]]:
...
def parse_delta(value: str) -> tuple[dict[str, int], dict[str, int], list[str], list[str]]:
    stats: dict[str, int] = {}
    ...
    for piece in re.split(r"[，,、;；]", normalized):
        ...
        if piece.startswith("标签："):
            tags.extend(split_slash(piece.split("：", 1)[1]))
            continue
        if piece.startswith("结局倾向："):
    ...
    return stats, major_stats, tags, route_add
...
def make_achievement_ids(paragraphs: list[str]) -> dict[str, str]:
    ...
    for line in paragraphs:
        if line == "十二、成就":
            collecting = True
            continue
        if collecting and line.startswith("十三、"):
            break
        if collecting:
...
def option_to_choice(option: dict, idx: int, achievement_ids: dict[str, str]) -> dict:
    choice_id = ["a", "b", "c"][idx]
    stats, major_stats, delta_tags, route_add = parse_delta(option.get("数值变化", ""))
    tags = split_slash(option.get("选项标签", "")) + delta_tags
...
def parse_compact_option(line: str) -> dict | None:
    match = re.match(r"^([ABC])\.\s*(.*?)｜反馈：(.*?)｜数值变化：(.*)$", line)
    if not match:
        return None
    _, text, feedback, delta = match.groups()
    return {"按钮文案": text.strip(), "反馈文案": feedback.strip(), "数值变化": delta.strip()}
...
def read_events(paragraphs: list[str], achievement_ids: dict[str, str]) -> list[dict]:
    ...
    for line in paragraphs:
        ...
        if current is None:
            continue
        compact = parse_compact_option(line)
        if compact:
            current["options"].append(compact)
            current_option = None
            continue
...
def read_settings(paragraphs: list[str]) -> dict:
    ...
    for line in paragraphs:
        ...
        elif line.startswith("系统提醒：") or line.startswith("开场系统吐槽："):
    ...
    whole_text = "\n".join(paragraphs)
    initial_match = re.search(r"通用数值：\n(.*?)\n\n专业专属数值：\n(.*?)\n\n隐藏数值：\n(.*?)(?:\n|$)", whole_text, re.S)
    if initial_match:
        for name, raw in iter_stat_pairs(initial_match.group(1)):
            if name in GLOBAL_STAT_MAP:
                initial_stats[GLOBAL_STAT_MAP[name]] = raw
        for name, raw in iter_stat_pairs(initial_match.group(2)):
            if name in MAJOR_STAT_MAP:
                initial_major_stats[MAJOR_STAT_MAP[name]] = raw
        for name, raw in iter_stat_pairs(initial_match.group(3)):
            if name in GLOBAL_STAT_MAP:
                initial_stats[GLOBAL_STAT_MAP[name]] = raw
...
def read_endings(paragraphs: list[str]) -> list[dict]:
    ...
    in_endings = False
    for line in paragraphs:
        if line.startswith("九、终局结局"):
            in_endings = True
            continue
        if in_endings and line.startswith("十、"):
            break
        if not in_endings:
            continue
        ...
        if line.startswith("结局名："):
            current["title"] = f"电气工程·{field_value(line)}"
    ...
    for ending in endings:
        ...
        ending.setdefault("title", ending.get("id", "电气工程结局"))
        ending.setdefault("description", "")
        ending.setdefault("condition", {})
        ending.setdefault("shareText", f"我在电气工程副本里活成了：{ending['title']}。")
...
def update_major_config(
    ...
    for major in majors:
        ...
        seen_stats = {stat["key"] for stat in major["majorStats"]}
        if "powerRespect" not in seen_stats:
            major["majorStats"].append(
                {
                    "key": "powerRespect",
                    "name": "强电敬畏",
                    "initialValue": settings["initialMajorStats"].get("powerRespect", 18),
                    "description": "对强电安全、电网系统和电力工程责任的真实敬畏。",
                }
            )
        for stat in major["majorStats"]:
            ...
            if stat["key"] == "circuitControl":
                stat["name"] = "电路脑"
                stat["description"] = "理解电路、电机、电力系统等硬核内容的能力。"
            elif stat["key"] == "smokeRisk":
                stat["name"] = "实验存活率"
                stat["description"] = "在接线、测量、看波形、写报告中不崩溃的概率。"
            elif stat["key"] == "stateGridObsession":
                stat["name"] = "国网执念"
                stat["description"] = "对国家电网、稳定路线和铁饭碗叙事的执念程度。"
            elif stat["key"] == "powerRespect":
                stat["name"] = "强电敬畏"
                stat["description"] = "对强电安全、电网系统和电力工程责任的真实敬畏。"
...
def main() -> None:
    ...
    word_achievements = [
        {
            ...
            "majorId": MAJOR_ID,
            "title": title,
            "description": f"在电气工程副本中解锁「{title}」。",
            "condition": {"type": "flag", "key": achievement_id},
            "shareText": f"我在电气工程副本里解锁了：{title}。",
        }
    ...
    print(f"imported {len(new_events)} electrical events, {len(endings)} endings, {len(achievements)} achievements")
