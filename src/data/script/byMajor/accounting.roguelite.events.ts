...
export const ACCOUNTING_CORE_EVENTS: AccountingEvent[] = [
  ...
  e("accounting_core_03_judgment", "y2s1", "main", "题里少了一个关键条件", "题目只说“商品已发出”，没说客户是否已经取得控制权。以前看见业务就写分录，现在老师先问你：为什么能在这时候确认收入？", [
    o("a", "逐句检查，看看确认条件到底齐没齐", "你在卷面写下“若控制权已转移”。老师没说对错，只在旁边补了五个字：“那你怎么证明？”", -7, { knowledge: 9, traits: { riskSensitivity: 10, ruleResearch: 8 }, flags: ["accounting_judgment_basis"], routes: { audit: 4, tax: 3, cpa: 3 }, proof: "note" }),
  ...
  e("accounting_core_04_fork", "y2s2", "main", "成本、税法、审计、财管同时留了作业", "成本会计要做分摊，税法要查规定，审计要做底稿，财管要算方案。四位老师都说作业不难，合在一起却足够占满整个期末周。", [
  ...
  e("accounting_core_05_route", "y3s1", "main", "CPA网课、实习和考公撞在这一周", "CPA 网课一下更新了六节，事务所在招寒假实习，企业财务岗发来 Excel 测试，考公群又提醒岗位表快出了。你不可能四件事都认真准备。", [
    o("a", "去事务所，看看审计底稿到底怎么做", "网课暂停在第十二节。你换来一份客户资料包，里面有三个最终版和一张拍歪的发票。", -8, { opportunity: 10, knowledge: 5, traits: { stressTolerance: 7, sourceTracing: 7 }, flags: ["accounting_route_audit"], closes: ["civil"], routes: { audit: 12 }, proof: "timeline" }),
  ...
  e("accounting_core_06_materials", "y3s2", "main", "带教说资料都在共享盘，你找了半小时", "合同在文件夹 A，发票还躺在群聊里，付款截图跟在一条语音后面，最新表格叫“最终版_新”。资料是齐了，只是没人知道它们分别在哪。", [
  ...
  e("accounting_core_07_conflict", "y4s1", "main", "秋招突然提前，考试和论文却没让路", "事务所终面、企业月结实习、考试冲刺和毕业论文全挤在同一周。四件事都重要，但你最多只能认真做好两件。", [
    o("a", "保事务所终面，也把实习任务做完", "错过两节冲刺课，换来一次盘点和一轮终面。简历多了一行，错题本也多了两页空白。", -9, { opportunity: 10, knowledge: 4, traits: { stressTolerance: 9 }, flags: ["accounting_lock_audit"], routes: { audit: 12 }, proof: "timeline" }),
  ...
  e("accounting_core_08_finish", "y4s2", "main", "实习最后一天，你要填“未完成事项”", "毕业论文交了，实习资料也该移交了。交接表最后一栏让你写清哪些账还没对上、哪些原件还没收到。毕业之前，最后一项任务还是把没做完的事说明白。", [
...
export const ACCOUNTING_ROUTE_EVENTS: AccountingEvent[] = [
  ...
  paired("accounting_audit_04", "y4s1", "audit", "抽样没查出问题，旁边一笔日期却不对", "规定抽查的记录都核完了，旁边一笔金额不大，日期却早了整整一个月。项目群在问几点能下班，你得决定要不要继续查。", ["多查三笔，再发给带教看", "带教看完只回：“范围扩大到十笔。”晚饭延期，但你确认了那行异常不是看错。", { opportunity: 10, knowledge: 8, traits: { riskSensitivity: 11, reviewPatience: 8 }, routes: { audit: 11 }, flags: ["accounting_audit_anomaly"], proof: "timeline" }], ["按抽样范围收工，把那笔单列", "底稿按时交，那笔早一个月的记录被你单独写在最后。你没有无限往下查，也没有装作没看见。", { traits: { boundarySense: 10, riskNote: 7 }, routes: { audit: 6, survival: 3 }, proof: "note" }]),
  ...
  paired("accounting_enterprise_04", "y4s1", "enterprise", "昨天说补签的人今天又来了", "实习快结束，经办人把另一张缺附件的单子放下：“和昨天一样先过呗。”昨天那张的签字，此刻仍在路上。", ["把两张缺件一起发群里问", "经办人终于补齐昨天那张，今天这张也没有继续拖到以后。", { opportunity: 9, traits: { boundarySense: 10, communication: 8 }, routes: { enterprise: 10 }, flags: ["accounting_enterprise_trust"], proof: "note" }], ["最后帮一次，别卡交接", "单据先录进去了。交接表第一条也随之变成：继续催经办人补两张附件。你帮的“最后一次”又给下一位留下了任务。", { traits: { overResponsibility: 8, deliveryFirst: 7 }, routes: { enterprise: 5 }, proof: "team" }]),
  ...
  paired("accounting_tax_01", "y2s2", "tax", "对方只问能不能做，却什么资料都没给", "案例里只有一句“公司准备这么处理”，合同、发票和付款安排都没有。组员已经开始搜索答案，但缺少这些信息，任何结论都可能要改。", ["先列资料清单，再查对应规定", "你搜得比别人慢，但没有在合同和发票都缺席时硬给结论。", { knowledge: 8, traits: { ruleResearch: 10, sourceTracing: 8 }, routes: { tax: 9 }, flags: ["accounting_tax_materials"], proof: "timeline" }], ["按最常见情形先给结论", "展示很顺，老师追问“如果合同条款不同呢”，你们只好当场给结论补前提。", { traits: { templateReliance: 8 }, routes: { tax: 4 }, proof: "closer" }]),
...
const randomRows: Array<[string, string, string, string, string, string, string]> = [
  ...
  ["03", "组员发来“最终版2真的最终”", "文件修改时间比群里的“最终”晚了十分钟。", "先确认哪份是底稿，再开始合并", "大家终于在同一份文件上继续改，没有再把旧数据混回来。", "两个版本都打开，肉眼找不同", "你在第七行发现一个金额改过，第八行开始怀疑自己漏了更多。两个“最终版”谁也不肯主动承认过期。"],
...
export const ACCOUNTING_HIDDEN_EVENTS: AccountingEvent[] = [
  ...
  e("accounting_hidden_tax_entry", "y4s1", "hidden", "老师把你的缺件清单转给了项目组", "你几次都不肯在合同和发票没到时回答“能不能”。这次老师发来一包真实资料：“先看看还缺什么，别急着给结论。”", [
    o("a", "逐张核日期，把缺的继续列出来", "合同和发票差了两个月，付款截图又少了对方名称。你把三处圈出来，没有在资料不齐时替项目负责人说“可以”。", -6, { knowledge: 9, opportunity: 10, traits: { ruleResearch: 10, boundarySense: 9 }, flags: ["accounting_callback_tax"], routes: { tax: 11 }, experiences: ["第一次参与真实税务资料核对"], proof: "note" }),
  ]),
