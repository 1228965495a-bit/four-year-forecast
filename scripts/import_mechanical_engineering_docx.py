...
MAJOR_ID = "mechanical_engineering"
DOCX_PATH = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/Users/zhenzhen/Downloads/B01-机械工程-完整四年脚本补强包.docx")
...
MAJOR_STAT_MAP = {
    "制图负荷": "drawingLoad",
    "公差敬畏": "toleranceRespect",
    "工厂适应度": "factoryAdaptability",
    "图纸债务": "drawingDebt",
    "力学阴影": "mechanicsShadow",
    "转码幻听": "codingHallucination",
}

HIDDEN_STAT_MAP = {
    "转码冲动": "codingImpulse",
    "临场稳重": "shopFloorComposure",
}

MAJOR_STAT_SCALE = {
    "drawingLoad": 3,
    "toleranceRespect": 3,
    "factoryAdaptability": 3,
    "drawingDebt": 1,
    "mechanicsShadow": 1,
    "codingHallucination": 1,
}

MAJOR_STAT_META = {
    "drawingLoad": ("制图负荷", "三视图、装配图、尺寸标注、明细表和毕业设计共同堆出来的画图压力。", 18),
    "toleranceRespect": ("公差敬畏", "对 0.1 毫米、轴承、螺栓、公差配合和机械细节的敬畏程度。", 12),
    "factoryAdaptability": ("工厂适应度", "面对金工实习、车间、机床、铁味地图和制造现场时的适应程度。", 16),
    "drawingDebt": ("图纸债务", "工程制图、课程设计、CAD 和毕设图纸积累的债务。", 0),
    "mechanicsShadow": ("力学阴影", "理论力学、材料力学、受力分析带来的心理阴影。", 0),
    "codingHallucination": ("转码幻听", "刷到转码帖后脑内循环“要不转码”的强度。", 0),
}

ROUTE_MARKER_MAP = {
    "transfer_major_hint": [],
    "cross_code_hint": ["mechanical_engineering_route_coding_product"],
    "vehicle_robotics_track": ["mechanical_engineering_route_vehicle_new_energy"],
    "postgraduate_track": ["mechanical_engineering_route_postgrad"],
    "solid_graduation": ["mechanical_engineering_route_manufacturing"],
    "clear_path_reflection": [],
}
...
def clean_bullet(value: str) -> str:
    return value.strip().removeprefix("•").removeprefix("-").strip()
...
def make_achievement_ids(paragraphs: list[str]) -> dict[str, str]:
    ...
    for line in paragraphs:
        ...
        if in_achievements and line.startswith("十一、"):
            break
        if in_achievements:
            names.append(clean_bullet(line))
...
def option_to_choice(option: dict, idx: int, achievement_ids: dict[str, str]) -> dict:
    choice_id = ["a", "b", "c"][idx] if idx < 3 else f"choice_{idx + 1}"
    tags = split_slash(option.get("获得标签", "") or option.get("标签", ""))
...
def read_events(paragraphs: list[str], achievement_ids: dict[str, str]) -> list[dict]:
    ...
    for line in paragraphs:
        ...
        if re.match(r"^选项[ABC]：?$", line):
            ...
            continue
        if line.startswith("默认数值变化："):
            current["options"] = [
                {
                    "按钮文案": "经历这个机械小插曲",
                    "反馈文案": current.get("事件描述", ""),
                    "数值变化": field_value(line),
                    "标签": current.get("标签", ""),
                }
            ]
            continue
        if current_option is not None:
            for key in ["按钮文案", "反馈文案", "数值变化", "获得标签", "路线标记"]:
...
def read_settings(paragraphs: list[str]) -> dict:
    ...
    for line in paragraphs:
        ...
        elif line.startswith("通用数值：") or line.startswith("专业专属数值：") or line.startswith("隐藏数值："):
...
def read_gg_events(paragraphs: list[str]) -> list[dict]:
    ...
    for line in paragraphs:
        ...
        if line.startswith("触发条件："):
            ...
            current["title"] = title
            current["semester"] = "y4s1" if "制图" in title else "y3s2"
        elif line.startswith("中途结算文案："):
    ...
    for event in events:
        title = event.get("title", "机械工程中途警报")
        tags = event.get("tags", [])
        ...
        options = [
            ...
            },
            {
                ...
                "text": "先停机检修，再继续上工",
                "feedback": "你没有否定机械工程，只是承认图纸、力学和车间都会消耗人类耐久。系统记录：停机检修也是工程能力。",
                "resultText": "你没有否定机械工程，只是承认图纸、力学和车间都会消耗人类耐久。系统记录：停机检修也是工程能力。",
...
def read_endings(paragraphs: list[str]) -> list[dict]:
    ...
    for line in paragraphs:
        ...
        if line.startswith("结局名："):
            current["title"] = f"机械工程·{field_value(line)}"
    ...
    return [
        {
            ...
            "majorId": MAJOR_ID,
            "title": title,
            "description": f"在机械工程副本中解锁「{title}」。",
            "condition": {"type": "flag", "key": achievement_id},
            "shareText": f"我在机械工程副本里解锁了：{title}。",
        }
...
def main() -> None:
    ...
    print(f"imported {len(new_events)} mechanical engineering events, {len(gg_events)} gg checks, {len(endings)} endings, {len(achievements)} achievements")
