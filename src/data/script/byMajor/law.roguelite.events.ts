...
export const LAW_ROGUELITE_EVENTS = [
  event("law_resource_y1s1_001", "y1s1", "第一次闭卷周，辩论队偏要今晚招新", "法理学小测在周五，辩论队试训在周四晚。你的精力只够认真准备一边，另一边最多留下一个礼貌的背影。", [
    option("a", "保住小测，告诉辩论队这次不参加", "你在试训群里说下次再来。法理学分数保住了，但这次招新的名单里没有你。", { energy: -9, professional: 8, opportunity: -3, persona: { ruleSensitivity: 5, realityPlanning: 3 }, routes: { academic: 5 }, evidence: "rule_keeper" }),
    ...
    option("c", "小测只复习重点，试训也只做基本准备", "两边的表现都很普通，但你没有为了一周的任务熬到天亮。", { energy: 5, professional: 1, opportunity: -1, persona: { ambiguityTolerance: 5, realityPlanning: 4 }, routes: { survival: 6 }, evidence: "planner" }),
  ]),
  ...
  event("law_resource_y1s2_001", "y1s2", "宪法论文、法律援助值班和六级挤在同一周", "三件事都很重要，你的日历却只剩两个晚上。全部认真做完已经不可能，你只能决定先保哪一项。", [
    ...
    option("c", "先保六级和睡眠，另外两项只做基本要求", "这周没有漂亮的法学经历，但六级复习没有中断，你也没有继续欠睡眠。", { energy: 3, professional: -2, opportunity: -2, persona: { realityPlanning: 8, stressTolerance: 4 }, routes: { survival: 7, detour: 2 }, evidence: "planner" }),
  ]),
  ...
  event("law_resource_y2s1_002", "y2s1", "四十八小时后交案例分析，学姐同时问你要不要参赛", "现有作业已经足够让周末消失，参赛却可能带来老师推荐。你能借来的只有时间，利息由下周的自己偿还。", [
    ...
    option("c", "和同学交换资料，各自负责一部分", "你们各自整理一半，再互相核对。作业按时交了，两个人也都保住了半个周末。", { energy: -7, professional: 6, opportunity: 4, persona: { responsibility: 5, ambiguityTolerance: 6 }, routes: { survival: 5 }, evidence: "mediator" }),
  ]),
  ...
  event("law_resource_y3s2_001", "y3s2", "保研论文、考公资料和律所实习同时向你招手", "这不是兴趣测试，而是三条路线开始要求真金白银的时间。继续平均用力，最可能得到三份平均水平的焦虑。", [
    ...
    option("c", "接受律所实习，暂停论文和考公准备", "实习开始占掉大部分时间，你第一次真正接触检索、改合同和反复改版本。论文和考公暂时只能往后排。", { energy: -19, professional: 10, opportunity: 10, persona: { realityPlanning: 9, evidence: 1 }, routes: { firm: 15 }, evidence: "evidence" }),
  ]),
  event("law_resource_y3s2_002", "y3s2", "法律援助来了真实求助，期末复习只剩四天", "求助者的问题无法延期，考试也不会因为你富有同情心而少考一章。你必须决定这周谁先得到完整的你。", [
    ...
    option("b", "专心备考，把求助转给有时间的同学", "你把情况和已经整理的信息一起转了出去。求助没有被搁置，你也保住了最后四天复习时间。", { energy: -10, professional: 10, opportunity: -2, persona: { realityPlanning: 8, responsibility: 2 }, routes: { academic: 7, civil: 4 }, evidence: "planner" }),
  ...
  event("law_resource_y4s1_002", "y4s1", "一个不完美的 offer，撞上还没出结果的考试", "offer 要求两天内答复，考试结果和更想去的岗位都还在路上。稳定、可能性和继续等待，不能同时拥有。", [
    ...
    option("b", "拒绝 offer，继续等更想去的岗位", "你没有提前定下来，接下来也要承担可能继续没有结果的风险。每天查邮箱，成了新的日常。", { energy: -12, professional: 4, opportunity: 5, persona: { idealDrive: 11, ambiguityTolerance: 8 }, routes: { academic: 4, civil: 5, detour: 4 }, evidence: "advocate" }),
  ...
  event("law_resource_y2s2_003", "y2s2", "模拟法庭队友赛前失联，你的转专业材料还差一页", "队友负责的质证稿一片空白，转专业申请今晚关窗。救场意味着材料可能来不及，先救自己意味着全队一起沉默。", [
    ...
    option("c", "只补质证框架，同时把失联情况告诉领队", "你补上了比赛必须用的框架，也让领队知道哪部分原本由失联队友负责。你没有默默把所有工作全接下来。", { energy: -10, professional: 6, opportunity: 2, persona: { evidence: 3, responsibility: 5, realityPlanning: 7 }, routes: { advocacy: 4, survival: 4 }, evidence: "evidence" }),
  ]),
...
export const LAW_ROGUELITE_RANDOM_EVENTS = [
  ...
  crisis("law_crisis_y3s2_exam_accuracy", "y3s2", "法考正确率连续三天向下，你开始刷经验贴求判决", "三百道题刷完，正确率从 52% 稳定降到 41%。收藏夹里已经有二十七篇上岸经验，脑子里的知识点正在互相撤销。", {
    ...
    option("c", "放慢法考进度，不用这周的正确率决定以后", "你从计划里删掉三套卷，只复盘已经做错的题，也恢复了正常睡眠。考试还在，但这次低分不等于整条路都失败。", { energy: 8, professional: 4, opportunity: -3, persona: { realityPlanning: 10, ambiguityTolerance: 6 }, routes: { survival: 6, civil: 3 }, evidence: "planner" }),
  ]),
  ...
  incident("law_incident_y2s2_001", "y2s2", "老师考前四十八小时宣布：开卷，但只能带一本书", "全班先欢呼，再发现一本到场意味着目录、索引和贴签将共同决定命运。开卷考试正式转型为书本导航竞赛。", [
    option("a", "重做目录和索引，把重点位置标清楚", "你贴了不同颜色的标签，考试时很快就翻到了对应章节。准备时间很长，至少没有浪费在考场上。", { energy: -8, professional: 6, opportunity: 0, persona: { ruleSensitivity: 5, realityPlanning: 4 }, routes: { academic: 4 } }),
