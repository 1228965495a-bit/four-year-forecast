...
import { CLINICAL_PERSONAS, CLINICAL_ROUTE_DEFINITIONS, type deriveClinicalResult } from "@/lib/clinicalMedicineRoguelite";
import type { ReplayRecommendation } from "@/lib/replaySystem";
import { ReplayNextLives } from "./ReplayNextLives";

type ClinicalResult = ReturnType<typeof deriveClinicalResult>;

export function ClinicalMedicineOutcomeView({
  ...
  const hiddenSeen = game.seenEvents.filter((id) => id.startsWith("clinical_hidden_")).length;
  return (
    <PhoneFrame
      topBar={<header className="v4-topbar">
        <button className="v4-icon-button" aria-label="返回首页" onClick={onHome}><ArrowLeft size={19} /></button>
        <div className="min-w-0 flex-1"><div className="v4-title text-[18px]">临床医学本科档案</div><div className="mt-0.5 text-[11px] text-[var(--v4-muted)]">第 {progress.playCount} 周目 · 已完成</div></div>
      ...
      bottomBar={<footer className="v4-outcome-actions">
        ...
        <button className="v4-primary" onClick={onShare}><Share2 size={17} />发给医学生</button>
      </footer>}
    >
        ...
        <article className="v4-law-result">
          <section className="v4-law-poster">
            <div className="v4-outcome-poster-head">
              <div className="flex items-center gap-2"><MajorMark id="clinical_medicine" size={42} /><div><div className="text-[10px] font-bold opacity-70">你的主路线结局</div><div className="text-[12px] font-bold">{result.route.title}</div></div></div>
              <Sparkles size={22} aria-hidden />
            </div>
            <div className="v4-law-route-ending"><Route size={14} /><span>{result.route.ending}</span></div>
            <div className="v4-outcome-eyebrow">你在医学院活成了</div>
            ...
            <div className="v4-outcome-stamp">五年一局 · 临床路线随机构筑</div>
          </section>

          <button className="v4-primary v4-outcome-share" onClick={onShare}><Share2 size={18} />生成我的医学生人格档案</button>
          ...
          <section className="v4-outcome-section">
            <div className="v4-outcome-section-title"><span>我的本科五年</span><small>事件有前因后果</small></div>
          ...
          <section className="v4-outcome-section">
            <div className="v4-outcome-section-title"><span>临床医学图鉴进度</span><small>一局只看见一部分</small></div>
            <div className="v4-law-progress-grid">
              ...
              <Progress label="特殊经历" value={progress.discoveredSpecialExperiences.length} total={18} />
              <Progress label="隐藏回收" value={hiddenSeen} total={8} />
              <Progress label="事件图鉴" value={game.discoveries.clinical_medicine?.length ?? game.seenEvents.length} total={eventTotal || 54} />
            </div>
            <div className="v4-law-codex-list">
              <UnlockRow label="已解锁路线" items={progress.unlockedRoutes.map((id) => CLINICAL_ROUTE_DEFINITIONS.find((route) => route.id === id)?.title).filter(Boolean) as string[]} />
              <UnlockRow label="已解锁人格" items={progress.unlockedPersonas.map((id) => CLINICAL_PERSONAS.find((persona) => persona.id === id)?.title).filter(Boolean) as string[]} />
